...
class MoveItFkDemo:
    def __init__(self):
        ...
        self.gripper_position = 227  # 初始化夹爪位置为关闭状态
        ...
        self.task_name = rospy.get_param('~task_name', 'pick_two')

        self.desired_execution_time = 5.0
        self.previous_z = 0.53
        ...
        self.init_pose_file('/home/pine/demo_ws/poses1.txt')
        
        # self.load_poses_from_file('poses.txt')
        self.initial_poses = {
            'handover': {
                'position': {'x': 0.209, 'y': -0.417, 'z': 0.402},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'pick_two': {
                'position': {'x': 0.28, 'y': -0.38, 'z': 0.28},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'pick_one': {
                'position': {'x': 0.27, 'y': -0.38, 'z': 0.28},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'press': {
                'position': {'x': 0.18, 'y': -0.38, 'z': 0.400},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'lift': {
                'position': {'x': 0.20, 'y': -0.37, 'z': 0.43},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'clothes': {
                'position': {'x': 0.25, 'y': -0.35, 'z': 0.32},
                'orientation': {'x': 0.0, 'y': -1.0, 'z': 0.0, 'w': 0.0}
            },
            'robot': {
                'position': {'x': 0.27, 'y': -0.38, 'z': 0.28},
        ...
        self.initial_joint_angles_degrees = {
            'pingpang': [-11.06, -107.84, -139.3, -114.41, -16.44, 177.8],
            'toothbrush': [-11.06, -107.84, -139.3, -114.41, -16.44, 177.8],
            'pour': [-11.06, -107.84, -139.3, -114.41, -16.44, 177.8],
            # 可以添加更多任务和对应的关节角度
        }
    ...
    def set_initial_pose(self):
        ...
        self.gripper_position = 227  # 更新夹爪位置
        self.gripper_publisher.publish(Float64MultiArray(data=[self.gripper_position]))  # 发布初始夹爪状态
...
if __name__ == "__main__":
    try:
        ...
        while not rospy.is_shutdown():
            ...
            # 加载新的位姿
            demo.load_poses_from_file('/home/pine/demo_ws/poses1.txt')  # Attempt to reload poses from file
